...
function ExecRequete($pRequete){
    // Nous allons récupérer la variable $connexion (définie dans la page base/config.php) grâce au mot clé "global"
    global $connexion;
    // Nous allons exécuter la requête grâce à la fonction mysqli_query
...
function requeteResultat($sql){
    ...
    if(!mysqli_connect_errno()){
        ...
        if( mysqli_num_rows($requete) == 0) {
            echo "dvnbsùpdwsv,µ<dsoqv,qµpo<bnqùpifnbùq";
            return false;
        }else{
